...
contract ShortsTracker is Governable, IShortsTracker {
    ...
    function updateGlobalShortData(
        ...
        (uint256 globalShortSize, uint256 globalShortAveragePrice) =
            getNextGlobalShortData(_account, _indexToken, _markPrice, _sizeDelta, _isIncrease);
    ...
    function getNextGlobalShortData(
        ...
        int256 realisedPnl = getRealisedPnl(_account, _indexToken, _sizeDelta, _isIncrease);
    ...
    function getRealisedPnl(address _account, address _indexToken, uint256 _sizeDelta, bool _isIncrease)
        public
        view
        returns (int256)
    {
        ...
        (uint256 size, /*uint256 collateral*/, uint256 averagePrice,,,,, uint256 lastIncreasedTime) =
            _vault.getPosition(_account, _indexToken, false);
    ...
    function _getNextGlobalAveragePrice(
        ...
        uint256 nextAveragePrice =
            (_nextPrice * _nextSize) / (hasProfit ? _nextSize - nextDelta : _nextSize + nextDelta);
