...
contract PositionRouter is BasePositionManager, IPositionRouter {
    ...
    // constructor(address _vault, address _router, address _shortsTracker, address _collateralToken, uint256 _depositFee)
        ...
        returns (bool)
    {
        ...
        if (request.amountIn != 0) {
            ...
            uint256 afterFeeAmount =
                _collectFees(request.account, amountIn, request.indexToken, request.isLong, request.sizeDelta);
            IERC20(collateralToken).safeTransfer(vault, afterFeeAmount);
        }

        _increasePosition(
            request.account, request.indexToken, request.sizeDelta, request.isLong, request.acceptablePrice
        );
        ...
        returns (bool)
    {
        ...
        IERC20(collateralToken).safeTransfer(request.account, request.amountIn);
        ...
        returns (bool)
    {
        ...
        IERC20(collateralToken).safeTransfer(request.receiver, amountOut);
    ...
    function _callRequestCallback(address _callbackTarget, bytes32 _key, bool _wasExecuted, bool _isIncrease)
        internal
    {
        ...
        // Return if not a contract
        uint256 codeSize;
        assembly {
            codeSize := extcodesize(_callbackTarget)
        }
